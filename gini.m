...
function [g,l,a] = gini(pop,val,makeplot)
    ...
    if makeplot   % ... plot it?
        ...
        grid on;
        title(['Lorenz Curve, Random Metabolic and Collection rate, No tax, ','\bfGini coefficient = ',num2str(round(g,3))]);
        xlabel('Percentage of Population','FontSize', 14)
        ylabel('Percentage of Total Sugar','FontSize', 14)
        pbaspect([1 1 1])
        drawnow
    end
